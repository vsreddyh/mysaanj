...
export default function Addkey({ setaddkey }) {
    ...
    async function addkey(){
        const response = await axios.post('http://192.168.29.80:3000/en/addkey',{key:key});
    ...
    return (
        <View className='flex items-center justify-center z-2 absolute w-full h-full bg-blue-600 opacity-80'>
            <View className='bg-white h-fit w-56'>
                ...
                <TextInput className='border border-black pl-1 rounded-lg mx-3' value={key} placeholder='Enter key' onChange={(e) =>
                    {
                        e.persist();
                ...
                <View className='flex-row'>
                    ...
                    <TouchableOpacity className='w-1/2 flex items-center justify-center' onPress={addkey}>
                        <Text>Submit</Text>
                    </TouchableOpacity>
                </View>
            </View>
